...
class ConvLayer:
    def __init__(self, name, mi, mo, apply_batch_norm, filter_sz = 5, stride = 2, act = tf.nn.relu):
...
class FractionallyStridedConvLayer:
    def __init__(self,name, mi, mo, output_shape, apply_batch_norm, filter_sz = 5, stride = 2, act = tf.nn.relu):
        self.W = tf.get_variable(
            "W%s" % name,
...
class DenseLayer(object):
    def __init__(self, name, M1, M2, apply_batch_norm, act = tf.nn.relu):
        self.W = tf.get_variable(
            "W%s" % name,
            shape = (M1, M2),
            initializer = tf.random_normal_initializer(stddev = 0.02)
...
class DCGAN:
    def __init__(self, img_length, num_colours, d_sizes, g_sizes):
        ...
        self.X = tf.placeholder(
            tf.float32,
            shape = (None, img_length, img_length, num_colours),
            name = 'X'
        )
        ...
        self.d_cost = 0.5 * (tf.reduce_mean(self.d_cost_real) + tf.reduce_mean(self.d_cost_fake))
        ...
        with tf.variable_scope("generator") as scope:
            ...
            for i in range(len(g_sizes['conv_layers'])):
                ...
                layer = FractionallyStridedConvLayer(
                    name, mi, mo, output_shape, apply_batch_norm, filter_sz, stride, act
                )
